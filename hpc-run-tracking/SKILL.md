...
4. **On complete**
   ...
   - Do not recommend moving/publishing before generating an integrity manifest.
...
6. **On publish or handoff**
   - Follow `checkpoint-integrity/SKILL.md` on the exact publish bundle.
...
commands. Install/run the helper in the same runtime environment that has
`wandb` available. For remote clusters, check the cluster profile for writable
scratch environment and launch details.
...
- or assemble the publish bundle, generate `CHECKPOINT_MANIFEST.json` (see `checkpoint-integrity/SKILL.md`), then upload / eval / hand off
```

Generate the manifest only after the publish bundle is final. It is a lightweight
byte-integrity record, not an approval or model-quality gate.
...
`checkpoint-integrity/SKILL.md` on the final bundle. Generate
`CHECKPOINT_MANIFEST.json` immediately before upload or copy.
...
CHECKPOINT_MANIFEST.json     # SHA-256 and size of each published file
...
- includes: README, TRAINING_LOG, CHECKPOINT_MANIFEST.json, assets, ...
...
- Generating `CHECKPOINT_MANIFEST.json` before the publish bundle is final — later README/config edits immediately make it stale
- Treating the integrity manifest as evidence that inference semantics or model quality are correct
